...
class CustomWebBrowser(QMainWindow):
    ...
    def __init__(self, whitelist=[homepage_path, 'http://www.neopets.com/', 'http://www.coolmathgames.com/', 'http://www.ixl.com/', 'http://mail.google.com/'], url=homepage_path):
        ...
        # Connect the load signals to their respective slots
        ...
        # Delay loading the homepage until the window has finished initializing
        self.homepage_loaded = False
        self.homepage_url = url
        self.homepage_whitelist = whitelist or []
    ...
    def load_url(self):
        # Create the QWebEngineView widget if it hasn't been created yet
        if self.web_view is None:
            self.web_view = QtWebEngineWidgets.QWebEngineView()
            self.web_view.setFixedSize(1080, 1080)
            self.web_view.loadStarted.connect(self.handle_load_started)
            self.web_view.loadProgress.connect(self.handle_load_progress)
            self.web_view.loadFinished.connect(self.handle_load_finished)
            self.central_layout.addWidget(self.web_view)
        ...
        # Load the URL in the QWebEngineView widget
